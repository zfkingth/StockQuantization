...
namespace Stock.JQData
{
    public class HandleFun
    {
        ...
        public void Update_allStock_price1d(StockContext db)
        {
            ...
            foreach (var sec in secList)
            {
                ...
                string res=qf.Get_price(db, sec.Code, dayCnt, UnitEnum.Unit_1d);
                Update_Single_securities_Price1d(db, sec.Code, res);
            }

        }


        public void Update_allStock_basicInfo(StockContext db)
        {
            //获取数据 
            var qf = new QueryFun();
        ...
        public void Update_Single_securities_Price1d(StockContext db, string code, string res)
        {
